...
class ChatUIManager {
  ...
  getDisplayNameByUserId(userId, fallbackName = '') {
    ...
    if (this.activeUsers && userId) {
      const active = this.activeUsers[userId] || this.activeUsers[String(userId)];
      if (active) {
        rawName = active.userName || active.displayName || rawName;
      }
    }
  ...
  getUserInfo(userId, userName) {
    ...
    const cacheKey = userId ? `id:${String(userId)}` : (userName ? `name:${String(userName).toLowerCase()}` : null);
    ...
    if (this.userProfiles && userId) {
      ...
      if (profile) {
        ...
        if (avatar && color) {
          ...
          if (cacheKey) this.avatarCache.set(cacheKey, info);
          return info;
        }
      }
    }
...
.message-actions button:hover {
    ...
    const renderUserItem = (user) => {
      ...
      return `
        <div class="active-user-item" data-user-id="${safeUserId}" style="display: flex; align-items: center; gap: 8px; margin: 6px 0; cursor: pointer; padding: 4px 2px; border-radius: 4px; transition: background 0.2s; opacity: ${opacity};">
          ${safeAvatar ? 
            `<img src="${safeAvatar}" class="active-user-avatar" style="width: 24px; height: 24px; min-width: 24px; min-height: 24px; max-width: 24px; max-height: 24px; aspect-ratio: 1 / 1; border-radius: 50%; object-fit: cover; flex-shrink: 0; display: block;">` :
  ...
  renderMessages(messages) {
    ...
    messages.forEach((msg, index) => {
        ...
        if (!existingEl) {
            ...
            // Update if needed
            const profile = this.userProfiles && (this.userProfiles[msg.userId] || this.userProfiles[String(msg.userId)]);
  ...
  createMessageElement(msg, messagesMap, isContinuation) {
    ...
    // Robust avatar/color lookup - check all sources (prefer resolved/updated name for legacy IDs)
    const userInfo = this.getUserInfo(msg.userId, resolvedName || msg.userName);
    const currentAvatar = userInfo.avatar || null; 
    ...
    if (currentAvatar && typeof currentAvatar === 'string' && currentAvatar.length > 5) {
        avatarHtml = `
            <div class="message-avatar-container" style="width: 32px; height: 32px; flex-shrink: 0; position: relative;">
                 <img src="${currentAvatar}" class="message-avatar" style="${avatarStyle} width:32px; height:32px; border-radius:50%; object-fit: cover; display:block;" 
  ...
  showReactionTooltip(badgeElement) {
    ...
    this.chatFirebase.messages.forEach(msg => {
        ...
        userMap[msg.userId] = this.getDisplayNameByUserId(msg.userId, msg.userName);
        }
    });
